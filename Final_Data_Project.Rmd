...
What we will do next is start creating our models.

First we will create a full model with all predictors. This will act as a control for using all of the available data, but contains the most parameters.

```{r, full-additive-model}
full_add_model = lm(Median_House_Value ~ ., data = housing_data_trn)
```

Next we will step through the model to see if any parameters are not needed from the above full model aren't necessarily useful. We will run this for both AIC and BIC.
...
And finally on the additive models, we wanted to hand pick some of the parameters that we believe would have the best change at producing a good model. We will remove `Tot_Rooms`, `Tot_Bedrooms`, and `Population` since they have >.9 correlation with `Households`. We are also removing `Latitude` and `Longitude` since we extracted useful data close to cities. We will also transform `Distance_to_coast` and `dist_to_nearest_city` with a log function since we saw that house value drops off logarthithmically with distance from coast. We also drop `near_a_city_200` since we already have another Boolean for 100km. We also choose to drop `Media_Age` since from

```{r, add-model-hand-picked}
hand_picked_add_model = lm(Median_House_Value ~ . - Tot_Rooms - Tot_Bedrooms - Population - Latitude - Longitude - Distance_to_coast + log(Distance_to_coast)- dist_to_nearest_city + log(dist_to_nearest_city) - near_a_city_200 - Median_Age, data = housing_data_trn)
```
...
rnames = c("Full", "AIC", "Hand Picked")
cnames = c("Train", "Test", "Num of Parameters")
results = matrix(rep(0, 9), nrow=3, dimnames = list(rnames, cnames))

results[1, 1] = rmse(housing_data_trn[,"Median_House_Value"], predict(full_add_model, newdata = housing_data_trn))
results[2, 1] = rmse(housing_data_trn[,"Median_House_Value"], predict(full_add_model_aic, newdata = housing_data_trn))
results[3, 1] = rmse(housing_data_trn[,"Median_House_Value"], predict(hand_picked_add_model, newdata = housing_data_trn))
results[1, 2] = rmse(housing_data_tst[,"Median_House_Value"], predict(full_add_model, newdata = housing_data_tst))
results[2, 2] = rmse(housing_data_tst[,"Median_House_Value"], predict(full_add_model_aic, newdata = housing_data_tst))
results[3, 2] = rmse(housing_data_tst[,"Median_House_Value"], predict(hand_picked_add_model, newdata = housing_data_tst))
results[1, 3] = get_num_params(full_add_model)
results[2, 3] = get_num_params(full_add_model_aic)
results[3, 3] = get_num_params(hand_picked_add_model)
...
Also AIC backward above

```{r}
hand_picked_int_model_aic = step(hand_picked_int_model, direction = "backward", trace = 0)
```
...
```{r}
rnames = c("Additive", "Fully Interactive", "Step")
cnames = c("Train", "Test", "Num of Parameters")
results = matrix(rep(0, 9), nrow=3, dimnames = list(rnames, cnames))

results[1, 1] = rmse(housing_data_trn[,"Median_House_Value"], predict(hand_picked_add_model, newdata = housing_data_trn))
results[2, 1] = rmse(housing_data_trn[,"Median_House_Value"], predict(hand_picked_int_model, newdata = housing_data_trn))
results[3, 1] = rmse(housing_data_trn[,"Median_House_Value"], predict(hand_picked_int_model_aic, newdata = housing_data_trn))
results[1, 2] = rmse(housing_data_tst[,"Median_House_Value"], predict(hand_picked_add_model, newdata = housing_data_tst))
results[2, 2] = rmse(housing_data_tst[,"Median_House_Value"], predict(hand_picked_int_model, newdata = housing_data_tst))
results[3, 2] = rmse(housing_data_tst[,"Median_House_Value"], predict(hand_picked_int_model_aic, newdata = housing_data_tst))
results[1, 3] = get_num_params(hand_picked_add_model)
results[2, 3] = get_num_params(hand_picked_int_model)
results[3, 3] = get_num_params(hand_picked_int_model_aic)
...
```{r, vif-1}
library(faraway)
mean(vif(full_add_model))
mean(vif(full_add_model_aic))
mean(vif(hand_picked_add_model))
```
...
Possibly change Nearest San Jose to San Fransisco, since those 2 cities are close and the p-value is quite large for nearest_citySan Jose
