...
What we will do next is start creating our models. We will refine and modify these models along the way and attempt to keep those which we deem significant.

First we will create a full model with all predictors. This is a somewhat natural choice as a model which uses all available predictors is a simple and usable starting point.
...
We see essentially no change to performance with the BIC and an improvement of the Variance Inflation Factor over the full model. We also see the hand-picked model performing decently well across most of the metrics but a substantial decreaase in VIF as well as a nice improvement in the number of indicators we have. It is a bit of a toss-up at this point as far as which models are best; reducing the numeber of predictors 40% is probably worth the performance decrease of about 10% in RMSE.
...
Our selected model is the model wherein we hand picked variables and performed transformation. wWe will now take a closer look at that model and it's performance.
...
-`Intercept` of $465300.88. This is the baseline value for the region of LA and all further modifcations will be made from that point.  
...
```   
Normality is suspect because of a rejection of the null hypothesis using the Shapiro Wilks test which assumes normality.
...
We can see above that the variance does not appear to be equaly distributed.
